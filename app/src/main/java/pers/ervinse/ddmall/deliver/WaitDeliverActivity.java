...
import pers.ervinse.ddmall.deliver.DeliverAdapter;
...
public class WaitDeliverActivity extends Activity {
    ...
    public void firstFresh(List<Order> list) {
        handler.post(new Runnable() {
            ...
            public void run() {
                //创建类别循环视图适配器,加载数据
                adapter = new DeliverAdapter(list, mContext);
                //循环视图加载适配器
        ...
        Thread initThread = new Thread(() -> {
            ...
            try {
                ...
                for (Order order : orderTemp) {
                    if (order.getOrderPayState().equals(2)) {
                        orders.add(order);
                    }
                }
