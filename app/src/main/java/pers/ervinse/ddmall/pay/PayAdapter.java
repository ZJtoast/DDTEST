...
public class PayAdapter extends RecyclerView.Adapter<PayAdapter.PayAdapterViewHolder> {
    ...
    public PayAdapter(List<Order> orderList, Context context) {
        this.orderList = orderList;
        this.mContext = context;
    }

    public void setPayList(List<Order> orderList) {
        this.orderList = orderList;
    }
    ...
    @NonNull
    @Override
    public PayAdapterViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = View.inflate(mContext, R.layout.item_pay, null);
        return new PayAdapter.PayAdapterViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull PayAdapterViewHolder holder, int position) {
        ...
        //通过图片名字获取图片资源的id
        try {
            OkhttpUtils.setImage(holder.medicine_image_pay, orderList.get(position).getCommodityID().toString(), mContext);
        } catch (IOException e) {
            Log.i("TAG", "图片传输错误");
        }
    }
    ...
    public class PayAdapterViewHolder extends RecyclerView.ViewHolder {
        private TextView merchant, drug_name;
        private Button pay_drug_button;
        private ImageView medicine_image_pay;
        private Button cancel_button;

        public PayAdapterViewHolder(@NonNull View itemView) {
            super(itemView);
            merchant = itemView.findViewById(R.id.merchant);
            drug_name = itemView.findViewById(R.id.drug_name);
            pay_drug_button = itemView.findViewById(R.id.pay_drug_button);
            medicine_image_pay = itemView.findViewById(R.id.medicine_image_pay);
            cancel_button = itemView.findViewById(R.id.cancel_button);
            cancel_button.setOnClickListener(v -> {
                Log.i(TAG, "删除订单按钮监听方法");
                //设置删除按钮点击之后的弹出对话框
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                new Thread() {
                                    ...
                                    public void run() {
                                        ...
                                        String responseJson = null;
                                        //获取要删除的商品ID
                                        String orderID = orderList.get(getLayoutPosition()).getOrderID().toString();
                                        try {
                                            //发送删除请求
                                            String url = PropertiesUtils.getUrl(mContext);
                                            responseJson = OkhttpUtils.doDeleteByToken(url + "/order/" + orderID, TokenContextUtils.getToken());
            ...
            //给支付按钮添加事件
            pay_drug_button.setOnClickListener(new View.OnClickListener() {
                @Override
                                    ...
                                    new Thread() {
                                        ...
                                        public void run() {
                                            ...
                                            try {
                                                ...
                                                Log.i(TAG, "支付商品响应解析对象:" + responseJson);
                                                Integer code = result.getCode();
                                                //删除成功
                                                if (responseJson != null) {
                                                    if (code.equals(200)) {
